...
describe('Products API PATCH handler', () => {
  ...
  it("should return 404 if product is not found", async () => {
    ...
    const req = createRequest({ id: "non-existent", name: "test", price: 10, stock: 10 });
    const res = await PATCH(req);
  ...
  it("should update product successfully", async () => {
    ...
    prisma.product.update.mockImplementation(((args: any) => Promise.resolve({ ...mockProduct, ...args.data })) as any);
  ...
  it("should clear Original Price", async () => {
    vi.spyOn(auth, "checkAdminAuth").mockResolvedValue(true);

    const mockProduct = { id: "123", name: "Old Name", price: 10, stock: 5, originalPrice: 20 };
    prisma.product.findUnique.mockResolvedValue(mockProduct as any);
    prisma.product.update.mockImplementation(async ({ data }: any) => ({ ...mockProduct, ...data }) as any);
  ...
  it("should update all fields", async () => {
    ...
    expect(data.name).toBe("New Name"); // Trimmed by route normally, let's see
